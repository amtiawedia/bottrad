#!/usr/bin/env python3
"""
🚀 Paper Trade Bot V2 - Dynamic Position Sizing + Better R:R
============================================================
ปรับปรุงจาก V1:
✅ Dynamic Position Sizing ตาม Confidence Level
✅ R:R 1:1.4 (SL 2.5%, TP 3.5%)
✅ ลบ Meme Coins ออก (ORDI, WIF, 1000PEPE, DOGE)
✅ ADX threshold เพิ่มเป็น 35
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# CONFIGURATION V2 - Dynamic Position Sizing + Better R:R
# ═══════════════════════════════════════════════════════════════════════════════

# 🎯 TOP QUALITY COINS - ลบ Meme Coins ออก!
COINS = [
    # Major (มั่นคง, Volume สูง)
...
# ⚠️ ลบออก: DOGE, ORDI, WIF, 1000PEPE (Meme coins = ผันผวนเกินไป)

# Trading Settings V2 - Better R:R
INITIAL_BALANCE = 4.50
LEVERAGE = 20  # 20x leverage
SL_PCT = 0.025  # 2.5% SL (= -50% ที่ 20x leverage)
TP_PCT = 0.035  # 3.5% TP (= +70% ที่ 20x leverage) Risk:Reward = 1:1.4
TIMEFRAME = '5m'
SCAN_INTERVAL = 30
MAX_POSITIONS = 3
ADX_THRESHOLD = 35  # เพิ่มจาก 30 เป็น 35 (เอา trend แรงๆ เท่านั้น)

# 🎯 Dynamic Position Sizing ตาม Confidence
POSITION_SIZE_MULTIPLIERS = {
    'LOW': 0.5,      # Confidence 50-70: 0.5x (ระวัง)
    'MEDIUM': 1.0,   # Confidence 70-85: 1x (ปกติ)
    'HIGH': 1.5,     # Confidence 85+: 1.5x (มั่นใจมาก)
}
...
# ═══════════════════════════════════════════════════════════════════════════════
# TRADINGVIEW PRO CHART - Premium/Discount Zones + FVG + RSI
# ═══════════════════════════════════════════════════════════════════════════════
...
class TradingViewChart:
    ...
    def create_pro_chart(df: pd.DataFrame, symbol: str, entry_price: float, 
        ...
        """สร้างกราฟ TradingView Pro Style พร้อม ICT Concepts"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# PAPER TRADE BOT V2 - Dynamic Position Sizing
# ═══════════════════════════════════════════════════════════════════════════════

class PaperTradeBotV2:
    def __init__(self):
        ...
        self.state_file = Path('paper_state_v2.json')
        self.load_state()
    ...
    def get_position_size(self, confidence: int) -> float:
        """คำนวณ Position Size ตาม Confidence"""
    ...
    def load_state(self):
        if self.state_file.exists():
            try:
                ...
                print(f"📂 โหลด State: Balance ${self.balance:.2f}, Positions: {len(self.positions)}")
            except:
                pass
    ...
    def analyze_signal(self, df: pd.DataFrame) -> dict:
        """วิเคราะห์สัญญาณพร้อม Confidence Score"""
        ...
        if 40 < rsi < 60:  # Neutral zone
            confidence += 5
        elif (rsi < 35 and trend_up) or (rsi > 65 and trend_down):  # Extreme with trend
            confidence += 10
    ...
    def open_position(self, symbol: str, side: str, price: float, reason: str, 
        ...
        # Print with confidence info
        emoji = "🟢" if side == "LONG" else "🔴"
        conf_emoji = "🔥" if level == "HIGH" else "⚡" if level == "MEDIUM" else "⚠️"
        
        print(f"\n{emoji} [PAPER V2] เปิด {side} {symbol}")
        ...
        # Create chart and send to Telegram
        sl_pct = SL_PCT * 100
        tp_pct = TP_PCT * 100
        multiplier = POSITION_SIZE_MULTIPLIERS[level]
        
        caption = f"""
📝 <b>PAPER TRADE V2 - เปิด {side}!</b>

⚠️ <i>จำลองเท่านั้น - ไม่ใช่เงินจริง!</i>

📊 <b>{symbol}</b>
├ 📍 Entry: <code>${price:,.4f}</code>
├ 🛡️ SL: <code>${sl:,.4f}</code> (-{sl_pct:.1f}%)
├ 🎯 TP: <code>${tp:,.4f}</code> (+{tp_pct:.1f}%)
...
"""
    ...
    def check_position(self, symbol: str, current_price: float) -> dict:
        """ตรวจสอบ Position ว่า TP/SL หรือยัง"""
    ...
    def close_position(self, symbol: str, result: dict, current_price: float):
        ...
        print(f"\n{emoji} [PAPER V2] ปิด {pos['side']} {symbol} - {result['type']}!")
        ...
        msg = f"""
{emoji} <b>PAPER TRADE V2 - ปิด {pos['side']}!</b>
...
"""
    ...
    def print_status(self):
        """แสดงสถานะปัจจุบัน"""
        print("\n" + "="*60)
        print("📊 PAPER TRADE BOT V2 - STATUS")
        print("="*60)
    ...
    async def run(self):
        """รัน Bot หลัก"""
        print("\n" + "="*60)
        print("🚀 PAPER TRADE BOT V2 - Dynamic Position Sizing")
        print("="*60)
        print(f"💰 Initial Balance: ${INITIAL_BALANCE}")
        print(f"📊 Coins: {len(COINS)} (No Meme Coins!)")
        print(f"⚙️ Settings: SL {SL_PCT*100}% | TP {TP_PCT*100}% | R:R 1:{TP_PCT/SL_PCT:.2f}")
        print(f"🎯 ADX Threshold: {ADX_THRESHOLD}")
        print(f"📈 Position Sizing: LOW=0.5x, MEDIUM=1x, HIGH=1.5x")
        print("="*60 + "\n")
        
        # Send startup message
        msg = f"""
🚀 <b>PAPER TRADE BOT V2 เริ่มทำงาน!</b>
...
└ 🪙 Coins: {len(COINS)} (No Meme!)

🎯 <b>Dynamic Position Sizing</b>
├ ⚠️ LOW (50-70%): 0.5x
├ ⚡ MEDIUM (70-85%): 1x
└ 🔥 HIGH (85%+): 1.5x
...
"""
        ...
        cycle = 0
        status_interval = LIVE_STATUS_INTERVAL * 2  # Every 30 cycles
...
║  🚀 PAPER TRADE BOT V2 - DYNAMIC POSITION SIZING                 ║
║  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ║
║  ✅ Dynamic Position Sizing (0.5x / 1x / 1.5x)                   ║
║  ✅ Better R:R 1:1.4 (SL 2.5%, TP 3.5%)                          ║
║  ✅ No Meme Coins (ลบ ORDI, WIF, PEPE, DOGE)                     ║
║  ✅ ADX >= 35 (เอาแค่ trend แรงๆ)                                ║
╚═══════════════════════════════════════════════════════════════════╝
    """)
